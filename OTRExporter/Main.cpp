...
std::shared_ptr<StormArchive> otrArchive;
...
static void ExporterParseFileMode(const std::string& buildMode, ZFileMode& fileMode)
{
	if (buildMode == "botr")
	{
		...
		if (DiskFile::Exists(otrFileName))
			otrArchive = std::shared_ptr<StormArchive>(new StormArchive(otrFileName, true));
		else
			otrArchive = StormArchive::CreateArchive(otrFileName, 40000);
...
static void ExporterProgramEnd()
{
	...
	std::shared_ptr<StormArchive> customOtr = StormArchive::CreateArchive(customOtrFileName, 4096);
